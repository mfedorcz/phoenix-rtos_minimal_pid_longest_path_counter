...
static int calcPath(process_info_t *process)
{
	process_info_t *child;
	int pathLength = 0;
	proc_lockSet(&process->lock);
	if(process->children != NULL){
		int longestPathCalc = -1;
		int mayBLongestPath;
		child = process->children;
		do{
			mayBLongestPath = calcPath(child);
			if(mayBLongestPath > longestPathCalc){
				longestPathCalc = mayBLongestPath;
			}
			child = child->next;
		}while (child != process->children);
		pathLength = longestPathCalc + 1;
	}
...
int posix_lab2(pid_t * pid_anws, int minPID)
{
	...
	* pid_anws = 0;
	...
	proc_lockSet(&posix_common.lock);
	process = lib_treeof(process_info_t, linkage, lib_rbMinimum(posix_common.pid.root)); //przypisanie pierwszego procesu do zmiennej(proces root)
	while(process != NULL)
	{
		currPID = process->process;
		if(currPID >= minPID)
		{
			int currPathLength = calcPath(process);
